...
**Provider rate limits cause silent protocol violations.** If the configured model provider hits rate limits (e.g., `openai-codex` free tier HTTP 429), the worker may exit cleanly (rc=0) before calling `kanban_complete` or `kanban_block`, triggering a "protocol violation" and task crash. The dispatcher sees `exit_code: 0` but no completion call.
- **Symptom:** Task events show `protocol_violation` + `gave_up` with `error: "worker exited cleanly (rc=0) without calling kanban_complete or kanban_block"`
- **Fix:** Before claiming kanban tasks, verify the model provider works. Switch from rate-limited providers (openai-codex free tier) to working ones:
  ```bash
  hermes config set model openrouter/auto
  hermes config set fallback_providers '["openrouter"]'
  ```
- **Prevention:** Use `web_search`/`web_extract` tools for research tasks instead of relying on Codex CLI; they don't depend on the Codex provider.

## MCP Server Installation on Hermes/Umbrel

When a kanban task involves installing MCP servers on Hermes running in Umbrel:

1. **Use `hermes mcp add` with stdio transport (npx)** — this saves config to Hermes internal MCP registry, not external files.
2. **Servers show as "disabled"** — this is EXPECTED for on-demand stdio servers. Hermes starts them automatically when tools are invoked.
3. **Persistent paths must be under `/opt/data`** — e.g., memory server: `--memory-file /opt/data/mcp/memory.json`
4. **API keys via `hermes auth add`** — not env files. Example: `hermes auth add brave-search` for Brave Search API key.
5. **All 7 servers installed in this session:**
   - `filesystem` — `/opt/data` access
   - `git` — `/opt/data` repo
   - `fetch` — web content
...
**CLI path in Umbrel containers:** The Hermes binary is at `/opt/hermes/bin/hermes` (or `/opt/hermes/hermes`). Use absolute path since it's not in PATH.
