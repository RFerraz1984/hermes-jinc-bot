...
def main():
    ...
    if resp.status_code == 200:
        data = resp.json()
        # Extract relevant rate limit fields
        rate_limit = data.get("rate_limit", {})
        out = {
            "label": data.get("label"),
            "limit_requests_per_minute": rate_limit.get("requests", {}).get("limit"),
            "remaining_requests_per_minute": rate_limit.get("requests", {}).get("remaining"),
            "limit_tokens_per_minute": rate_limit.get("tokens", {}).get("limit"),
            "remaining_tokens_per_minute": rate_limit.get("tokens", {}).get("remaining"),
            "limit_requests_per_day": rate_limit.get("requests_per_day", {}).get("limit"),
            "remaining_requests_per_day": rate_limit.get("requests_per_day", {}).get("remaining"),
        }
