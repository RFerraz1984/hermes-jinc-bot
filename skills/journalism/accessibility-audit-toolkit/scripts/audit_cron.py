#!/usr/bin/env python3
"""
Script standalone para execução via cron (no_agent=True).
Roda auditoria completa e entrega relatório via stdout para o Hermes entregar ao Telegram.
"""
...
async def main():
    """Executa auditoria para os sites alvo do Jornalista Inclusivo."""
    
    # Carrega URLs do arquivo de produção
    urls_file = Path("/opt/data/urls_production.txt")
    if urls_file.exists():
    ...
    output_dir = Path(f"/opt/data/audits/cron_{timestamp}")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    summary_lines = [f"🤖 **Auditoria Automática JINC** — {timestamp}\n"]
    
    for url in target_urls:
        try:
            logger.info(f"Iniciando auditoria: {url}")
