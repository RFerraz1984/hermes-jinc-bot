#!/usr/bin/env bash
# /opt/data/scripts/backup-hermes-selective.sh
...
echo "📦 Criando backup seletivo: ${OUT}"
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
    echo "⚠️  Arquivos/diretórios não encontrados (serão ignorados):"
    printf '   %s\n' "${MISSING[@]}"
fi
...
echo "🗂️  Backups mantidos: ${REMAINING}/30"
